...
import software.amazon.location.sample.util.BTN_TRACKING_BATTERY_SAVER
...
import software.amazon.location.sample.util.STOP_TRACKING_IN_BACKGROUND_BATTERY_SAVER
...
@LargeTest
class MainActivityBatterySaverTrackingTest {
    ...
    @Test
    fun startStopBatterySaverTracking() {
        ...
        composeTestRule.onNodeWithTag(BTN_TRACKING_BATTERY_SAVER).assertIsDisplayed()
        composeTestRule.onNodeWithTag(BTN_TRACKING_BATTERY_SAVER).performClick()

        Thread.sleep(DELAY_15000)
        composeTestRule.waitForIdle()
        composeTestRule.waitUntil(DELAY_15000) {
            uiDevice.findObject(By.text(STOP_TRACKING_IN_BACKGROUND_BATTERY_SAVER)) != null
        }
        composeTestRule.onNodeWithTag(BTN_TRACKING_BATTERY_SAVER).performClick()
        Thread.sleep(DELAY_5000)
        composeTestRule.waitForIdle()
        ...
        assertTrue(
            OUTPUT_MESSAGE,
            logcatOutput.contains("doWork getDeviceLocation")
        )
    }
